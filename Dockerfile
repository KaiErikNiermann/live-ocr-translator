...
RUN mkdir gtklib
RUN mkdir -p gtkapp/share/glib-2.0/schemas/
RUN mkdir -p gtkapp/share/icons/
RUN mkdir -p gtkapp/lib/gdk-pixbuf-2.0/
...
RUN cp -r gtklib/mingw64/share/icons/* gtkapp/share/icons
RUN cp -r gtklib/mingw64/lib/gdk-pixbuf-2.0 gtkapp/lib
RUN cp -r assets gtkapp
